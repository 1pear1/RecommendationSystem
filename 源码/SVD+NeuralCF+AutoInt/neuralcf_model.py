...
def run_experiment_with_split_method(split_method, split_method_name):
    ...
    print(f'\n{"="*80}')
    print(f'运行NeuralCF实验 - 数据划分方法: {split_method_name} ({split_method_en})')
    print(f'{"="*80}')
    ...
    model = NeuralCF(len(u_map), len(i_map), embedding_dim=config.model.latent_dim)
    trained_model = train_model(model, 'NeuralCF', split_method_name, split_method_en, train_loader, val_loader)
    predict_and_save(trained_model, 'NeuralCF', final_test_df, u_map, i_map, split_method_en, train_df['rating'].mean())

def print_performance_comparison():
    print(f'\n{"="*80}')
    print(' ' * 25 + 'NeuralCF模型性能报告')
    print(f'{"="*80}')
    ...
    results_file = os.path.join(config.experiment.results_dir, 'neuralcf_performance_comparison.csv')
